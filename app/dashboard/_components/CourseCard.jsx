...
function CourseCard({course,refreshData,displayUser=false}) {
  ...
  return (
    <div className='shadow-sm rounded-lg border p-2
     cursor-pointer mt-4 hover:border-blue-500'>
           ...
           {!displayUser&& <DropdownOption
            ...
            <div className='flex items-center justify-between'>
                <h2 className='flex gap-2 items-center
                 p-1 bg-blue-100 text-blue-500 text-sm rounded-sm'>
                    <HiOutlineBookOpen/>{course?.courseOutput?.course?.noOfChapters} Chapters</h2>
                <h2 className='text-sm bg-blue-100 text-blue-500 p-1 rounded-sm'>{course?.level}</h2>
